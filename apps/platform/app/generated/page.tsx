...
// A relationship field's target is declared either through its name (customerId
// -> Customer) or, when the name doesn't say it (currentTenant, assignedVendor),
// through the field's own declared type (reference:Tenant, relationship:Vendor).
// The declared target is matched as a whole singularized name, never as a
// substring, so e.g. "reference:Order" can't accidentally match a "WorkOrder" entity.
const relationshipType = /^(?:reference|relationship)\s*:\s*(.+)$/i;
export const relation = (field: Field, entities: Entity[]) => {
  const declared = relationshipType.exec(field.type);
  if (declared) {
    const target = singular(declared[1]);
    const match = entities.find((entity) => singular(entity.name) === target);
    if (match) return match;
  }
  return entities.find((entity) => singular(entity.name) === singular(field.name.replace(/_?id$/i, "")));
};
...
export const recordLabel = (record: RecordItem, entity: Entity, entities: Entity[]) => { const preferred = fields(entity).find((field) => /name|title|subject|email|label/i.test(field.name)) ?? fields(entity).find((field) => !relation(field, entities) && record[field.name]); return display(preferred ? record[preferred.name] : undefined); };
